...
/* ─── In-memory rate limiter ─────────────────────────────────────────────────
   Enforced only outside Vercel (local dev / single-process environments).
   On Vercel each invocation may land on a different instance, making a
   shared Map unreliable — the env check skips it rather than giving false
   confidence. The SSRF guard is the authoritative security layer.           */
...
export default async function handler(
  ...
  // Rate limit — skipped on Vercel where per-instance memory is unreliable
  if (!process.env.VERCEL) {
    try {
      ...
      // Never block a request due to rate-limit bookkeeping failures
    }
  }
  ...
  // Proxy fetch — maxRedirections:0 so 3xx is captured with Location intact
  const t0 = Date.now();
